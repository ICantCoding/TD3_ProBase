...
namespace TD3_Framework
{
    ...
    public class PackageAssetBundle
    {
        ...
        // private static string m_buildABTargetPath = Application.streamingAssetsPath;    //AB包打包生成目录
        // private static string m_abConfigAssetPath = "Assets/Framework/AssetBundle/Config/AssetBundleConfig.asset"; //AB包打包方式配置文件
        // private static string xmlPath = Application.dataPath + "/Framework/AssetBundle/Config/AssetBundleDependenceConfig.xml"; //AB包依赖xml文件
        // private static string bytePath = Application.dataPath + "/Framework/AssetBundle/Config/AssetBundleDependenceConfig.bytes"; //AB包依赖二进制文件

        [MenuItem("编辑器小工具/AssetBundle/打包配置文件生成(推荐)", false, 1)]
        private static void ProduceAssetBundleConfig()
        {
            ...
            abConfig.directoryABPathList.Add(new ABConfig.DirectoryAB()
            {
                ...
                DirectoryPath = "Assets/Framework/AssetBundle/Config"
            });
            UnityEditor.AssetDatabase.CreateAsset(abConfig, GlobalConfigData.AssetBundleConfigPath);
            UnityEditor.AssetDatabase.SaveAssets();
            UnityEditor.AssetDatabase.Refresh();
        }

        [MenuItem("编辑器小工具/AssetBundle/根据配置文件打包(推荐)", false, 2)]
        private static void BuildAssetBundle()
        {
            ...
            //读取打包策略对应的配置文件
            ABConfig abConfig = AssetDatabase.LoadAssetAtPath<ABConfig>(GlobalConfigData.AssetBundleConfigPath);
            ...
            //开始打包
            BuildPipeline.BuildAssetBundles(GlobalConfigData.AssetBundleBuildTargetPath, 
        ...
        private static void MakeDependenceRelationShip()
        {
            ...
            DirectoryInfo directoryInfo = new DirectoryInfo(GlobalConfigData.AssetBundleBuildTargetPath);
            ...
            foreach (string path in dict.Keys)
            {
                ...
                abBase.Path = path;
                abBase.Crc = Crc.StringToCRC32(path);
            ...
            //写入xml
            if (File.Exists(GlobalConfigData.AssetBundleDependenceXmlPath))
            {
                File.Delete(GlobalConfigData.AssetBundleDependenceXmlPath);
            }
            FileStream fileStream = new FileStream(GlobalConfigData.AssetBundleDependenceXmlPath, 
                FileMode.Create, 
            ...
            if (File.Exists(GlobalConfigData.AssetBundleDependenceBytePath))
            {
                File.Delete(GlobalConfigData.AssetBundleDependenceBytePath);
            }
            fileStream = new FileStream(GlobalConfigData.AssetBundleDependenceBytePath, 
                FileMode.Create,
                FileAccess.ReadWrite, 
                FileShare.ReadWrite);
